...
restaurant_children = categories.select do |category|
  category[:parents].include?("restaurants")
end

restaurant_children_aliases = restaurant_children.map do |category|
  category[:alias]
end

restaurant_grandchildren = categories.select do |category|
  !(category[:parents] & restaurant_children_aliases).empty?
end

restaurant_categories = restaurant_children + restaurant_grandchildren

united_states_restaurant_categories = restaurant_categories.select do |category|
...
united_states_restaurant_categories.each do |category|
  Category.create(title: category[:title], yelp_alias: category[:alias])
end

categories.each do |child|
  child[:parents].each do |parent_alias|
    child_category = Category.find_by(yelp_alias: child[:alias])
    parent_category = Category.find_by(yelp_alias: parent_alias)
    if child_category && parent_category
      Ancestry.create(child_id: child_category.id, parent_id: parent_category.id)
    end
  end
end